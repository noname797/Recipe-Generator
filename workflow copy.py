...
def ingredient_node(state):
    user_input = input("Enter your available ingredients, dietary preferences, allergies, cuisine, and flavor profiles: ")
...
def nutrition_node(state):
    ...
    for idx, recipe in enumerate(state.nutrition_info.recipes, 1):
        print(f"\nRecipe {idx}: {recipe.name}")
        print(f"Nutrition Profile: {recipe.nutrition_profile}")
        print(f"Suggested Add-ons: {recipe.suggested_addons}")
        print(f"Nutrition Insights: {recipe.nutrition_insights}")
...
def user_modification_node(state):
    user_mod_request = input("Would you like to modify the recipe? (e.g., reduce salt, swap an ingredient, adjust nutrition): ")
    if user_mod_request.strip():
...
def sentiment_node(state):
    user_feedback = input("How do you feel about your experience? (Optional feedback for sentiment analysis): ")
    agent = SentimentEscalationAgent(test_mode=state.test_mode)
    state = agent.analyze_sentiment(user_feedback, state)
